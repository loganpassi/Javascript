...
function createList(object, list) {
	...
	for (var n = object.firstChild; n != null; n = n.nextSibling) {
		...
		if (nodeLevel != -1) {
			...
			//create a list item to mach
			var listItem = document.createElement("li");
			listItem.id = "TOC" + n.id;
			
			//create hypertext link to the section heading
			...
			if (nodeLevel == prevLevel) {
				//append the entry to current list
...
function expandCollapse(e) {
	...
	//expand and collapse the source document to match the TOCList
	expandCollapseDoc();
}
